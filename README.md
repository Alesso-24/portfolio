...
The whole site uses a glass design system: `src/styles/glass.css` (variables + classes such as `.glass`, `.glass--read`, `.glass-lift`, `.glass-btn`), a static ambient layer behind the page, and a floating glass nav. Real `backdrop-filter` is used only on floating elements (nav, mobile menu, image viewer); everything else uses a cheap "glass look". Long text always sits on `.glass--read` (≈80 % opaque). The nav and image-viewer controls use **real refraction** (SVG `feDisplacementMap` + Snell's law, `src/scripts/glass-refract.ts`, Chromium only, with a performance governor); other browsers keep the frosted look. Falls back to solid surfaces with `prefers-reduced-transparency`, `prefers-contrast: more`, `forced-colors` and browsers without `backdrop-filter`. Research, decisions, measurements and how to extend it: `docs-source/LIQUID-GLASS.md`. The nav is a dock that contracts to icons while scrolling down and expands on scroll up or hover. How the project is worked on (git flow, QA, measurement traps, conventions): `docs-source/METODOLOGIA-DE-TRABAJO.md`.
