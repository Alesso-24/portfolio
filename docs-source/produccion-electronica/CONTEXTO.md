...
### 2026-09-21 (noche) — Ícono oficial de la app de KiCad
Alessandro pidió usar el ícono de la app de KiCad 10 (cuadrado azul redondeado, "Ki" blanca, punto naranja) en buena calidad. **Fuente:** `resources/linux/icons/hicolor/scalable/apps/kicad.svg` del repositorio oficial (gitlab.com/kicad/code/kicad), guardado en `docs-source/produccion-electronica/kicad/logo/kicad-app-icon-official.svg`; **licencia CC BY-SA 4.0** (© KiCad Project; atribuir si se redistribuye). Se renderizó a `public/images/docs/produccion-electronica/brand/kicad-app-icon.webp` (256×256, alfa, 5.7 KB) y sustituye al recorte anterior en la **pestaña de KiCad** (sin caja blanca, con sombra azul suave) y, en el **encabezado del panel de KiCad**, el logo completo modernizado (ver abajo). Se eliminaron `kicad-icon.webp` y `kicad-logo-sm.webp` (siguen en el historial y en `restore/*`; el SVG con texto de Wikimedia sigue en `docs-source/.../kicad/logo/`). Para regenerar: `sharp` con el SVG a `density: 1536` y `resize(256, 256)`.
**Logo completo con texto:** se investigó si existe una versión actualizada del logo con "KiCad" (Wikimedia Commons, repos de KiCad, la web y la instalación de KiCad 10): **no existe**; la web oficial sigue usando el logo de 2016 (`kicad_logo_paths.svg`) y el diseño nuevo solo se hizo para el ícono de la app. Se **compuso** uno moderno con piezas oficiales: el ícono de KiCad 10 en el lugar del cuadrado + las letras "Cad" vectoriales del logo oficial. Es una composición propia, **no un archivo oficial** (queda dicho en el comentario del SVG). Archivos: `docs-source/produccion-electronica/kicad/logo/kicad-logo-modernizado.svg` (fuente), `kicad-logo-oficial-web-2016.svg` (el original de kicad.org, por si se quiere volver) y `public/images/.../brand/kicad-logo.webp` (660×270, 12 KB). Pestaña = ícono pequeño (`kicad-app-icon.webp`); panel = logo completo (`kicad-logo.webp`, 147×60 en pantalla). Regenerar: `sharp` con el SVG a `density: 600` y `resize({ width: 660 })`. Licencia del ícono: CC BY-SA 4.0 © KiCad Project.
...
**Bug de la burbuja corregido** (rama `feat/glass-dock-iconos`, PR #74): la burbuja no encerraba la palabra de la sección activa; ahora sigue al enlace fotograma a fotograma (0 px de diferencia). De paso el regulador de rendimiento se recalibró (umbral 28 → 36 ms) porque se disparaba con el ruido normal. Ver LIQUID-GLASS.md §9.3.
