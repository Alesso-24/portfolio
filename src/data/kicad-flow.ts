  ...
  sections: [
    // ───────────────────────── 0 · Antes de empezar
    {
      ...
      steps: [
        ...
        {
          html: 'Antes de dibujar cualquier esquemático hace falta instalar <strong>KiCad FabLib</strong>: una librería de componentes pensada para fabricar PCBs en un Fab Lab estándar, con mapeo 1:1 a las piezas del inventario oficial de Fab Lab — reduce la fricción de aprender electrónica y da más confianza al prototipar rápido. Se instala desde <strong>Herramientas → Administrador de complementos y contenido</strong> (<code>Ctrl+M</code>).',
          blocks: [
    ...
    // ───────────────────────── 4 · Pulsadores
    {
      ...
      steps: [
        ...
        {
          html: 'El switch conecta <strong>V3.3</strong> al nodo <code>s3</code>. Desde ahí, una resistencia baja a <strong>GND</strong> — es una <strong>pull-down</strong>: mantiene el nodo en LOW cuando el botón no está presionado, en vez de dejarlo flotando. La otra rama sale del mismo nodo por una resistencia limitadora hacia un <strong>LED</strong> a GND — se enciende cada vez que el botón está presionado, como indicador visual. El nodo <code>sN</code> de cada módulo es lo que sale hacia el conector de salidas (siguiente sección).',
          blocks: [
            {
              kind: 'callout',
    ...
    // ───────────────────────── 7 · Editor de placas
    {
      ...
      steps: [
        ...
        {
          html: 'Antes de colocar nada se configuran las reglas de diseño en <strong>Configuración de la placa → Reglas de diseño → Requerimientos</strong>: se fija el <strong>margen mínimo de cobre en 0.4 mm</strong> — la separación mínima entre conductores, acorde al material y proceso de fresado de la placa — y se deja el ancho de pista predefinido también en 0.4 mm (con una opción alternativa de 0.8 mm para pistas de más corriente).',
          blocks: [
